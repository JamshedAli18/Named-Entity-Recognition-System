...
def download_spacy_model(model_name):
    try:
        ...
        try:
            ...
            st.info(f"Downloading {model_name} model... (This may take a minute)")
            subprocess.check_call([sys.executable, "-m", "pip", "install", model_name])
            st.success(f"Model {model_name} has been installed!")
            return True
    except Exception as e:
        st.error(f"Failed to download model: {e}")
        return False

# Now we can safely import spacy after ensuring the model is downloaded
@st.cache_resource
...
with st.spinner(f"Loading {model_name}... (This may take a minute on first load)"):
    nlp = load_model(model_name)
    
    if nlp is not None:
        st.sidebar.success(f"Model '{model_name}' loaded successfully!")
        
        # Import spacy and displacy now that we know spaCy is installed
        import spacy
        ...
        text_input = st.text_area("Paste your text here:", height=150, 
                                value="In January 2025, Microsoft Corporation announced a major acquisition of DataSphere Technologies for $8.5 billion, causing their stock to rise by 12% on the New York Stock Exchange.")
        ...
        st.error(f"Failed to load model {model_name}. Please try a different model.")
